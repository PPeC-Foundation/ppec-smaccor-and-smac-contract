...
contract AdCreator {
    ...
    address PPeCAddress = 0x1C66Eb15D6D7D252BBdf65e6F6D2E14c1620DadC; // PPeC contract address.
...
contract SmACV1 {
    ...
    function claim() public notOwners {
        ...
        // Revert the call if the reward exceeds
        // the coffer balance.
        if (Ads.reward > cofferBalance())
    ...
    function scam() public onlyFounder returns (bool success) {
        ...
        SmACCor(adCreatorAddress).updatePledged(Ads.promoter, 1000000000E18);
    ...
    virtual 
    {
        ...
        // to the contract than the budget required. 
        else if ((balance - Ads.budget) < 0) {
            // set the extra tokens to exclude from fees.
            extraToken = 0;
            // set the reward to the balance.
            reward = balance;
            // set the pledged amount to be reduced by.
            pledge = Ads.budget - balance;
        } 
